...
# Author: Dhinesh R
# Team: Infra Team
# Description: This Script uploads Jenkins logs to Azure Blob Storage account
...
for job_dir in "$JENKINS_DIR/jobs/"*/; do
    ...
    for build_dir in "$job_dir/builds/"*/; do
        ...
        # Check if log file exists and was created today
        if [ -f "$log_file" ] && [ "$(date -r "$log_file" +%Y-%m-%d)" == "$DATE" ]; then
            # Upload log file to Azure Blob Storage with the build number as the filename
            azcopy copy "$log_file" "https://$AZURE_STORAGE_ACCOUNT.blob.core.windows.net/$AZURE_STORAGE_CONTAINER/$job_name-$build_number.log"

            if [ $? -eq 0 ]; then
                echo "Uploaded: $job_name/$build_number to "https://$AZURE_STORAGE_ACCOUNT.blob.core.windows.net/$AZURE_STORAGE_CONTAINER/$job_name-$build_number.log""
            else
                echo "Failed to upload: $job_name/$build_number"
            fi
        fi
